...
export default function Rank({ studentNumber }: RankProps) {
  ...
  const [sortCriteria, setSortCriteria] = useState<'score' | 'ratings' | 'student_number'>('student_number');
  ...
  const handleSortChange = (criteria: 'score' | 'ratings' | 'student_number', order: 'asc' | 'desc') => {
      ...
      <div className="container space-y-4 pb-6 px-2 sm:px-0">
        <div className="flex flex-col sm:flex-row items-center justify-between mb-4 mx-5">
          ...
          <div className="flex items-center space-x-4 w-full sm:w-auto">
            ...
            >
              Sort by {sortCriteria === 'student_number' ? 'Number' : sortCriteria === 'score' ? 'Score' : 'Ratings'} ({sortOrder === 'asc' ? 'Asc' : 'Des'})
                        ...
                        <td className="p-4">
                          <div className="flex items-center">
                            {[...Array(MAX_RATING)].map((_, index) => {
                              ...
                              } else if (hasFractionalStar && index === fullStars) {
                                return (
                                  <svg
                            ...
                            <span className="ml-2 text-sm text-gray-600">{derivedRating}/5</span>
                          </div>
                        </td>
